...
const ListofFilters = () => {
  ...
  return (
    <ul className='text-xs lg:text-sm  flex lg:justify-around justify-center flex-wrap gap-2 w-full'>
        {!loading && <li key={1} onClick={()=>{selectCatHandler(null)}} className={`p-1 lg:p-2 lg:px-4 rounded-lg cursor-pointer ${Filter === null ? 'bg-black shadow-xl text-white' : 'text-zinc-600 bg-zinc-100 dark:bg-zinc-800 dark:text-zinc-300 transition-all ease-linear duration-300'} `} >All</li>}
        {!loading  &&  categories?.map((filter)=> <li key={filter._id} onClick={()=>{selectCatHandler(filter._id)}} className={`p-1 lg:p-2 lg:px-4 rounded-lg cursor-pointer ${Filter === filter._id ? 'bg-black shadow-xl text-white' : 'text-zinc-600 bg-zinc-100 dark:bg-zinc-800 dark:text-zinc-300 transition-all ease-linear duration-300'} `} >{filter.title}</li> )}
        {loading && [1,2,3,4,5].map((filter)=> <li key={filter._id} className={`p-1 lg:p-2 lg:px-4 rounded-lg w-[15%] h-8 bg-zinc-100 dark:bg-zinc-800 animate-pulse `} ></li>)}
    </ul>
  )
}
